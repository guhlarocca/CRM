...
class TimeRepositorio(Repositorio):
    def criar_membro(self, nome, email, telefone, profile_photo='default_profile.png'):
        try:
            ...
            # Inserir novo membro com ID calculado
            self.cur.execute("""
                INSERT INTO time (id, nome, email, telefone, profile_photo)
                VALUES (%s, %s, %s, %s, %s)
                RETURNING id
    ...
    def atualizar_membro(self, id, dados):
        try:
            self.cur.execute("""
                UPDATE time
                SET nome = %s,
